...
function displayResults(results, flags, policyDetails) {
  ...
  let finalOutput = `${LOG_TITLE_MULTIPLE(results.isInSupportWindow, results.expiringSoonCount)}\n\n`;
  if (results.mps.length > 1 && (flags.unsupported || flags.supported || flags.expiring)) {
    let tempList = [];
    if (flags.unsupported) {
      tempList.push.apply(tempList, results.mps.filter((result) => !result.isInSupportWindow));
    }
    if (flags.supported) {
      tempList.push.apply(tempList, results.mps.filter((result) => result.isInSupportWindow));
    }
    if (flags.expiring) {
      tempList.push.apply(tempList, results.mps.filter((result) => result.isExpiringSoon));
    }
